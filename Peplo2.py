...
#
    point_cloud_mat = sl.Mat(width, height, sl.MAT_TYPE.U8_C4)
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            cam.retrieve_image(mat_l, sl.VIEW.RIGHT_GRAY)
            ...
            vl = np.shape(left_image)[0]
            hl = np.shape(left_image)[1]

            f_in_img = np.zeros(np.shape(right_image), dtype=np.complex_)


            lf_in_img = np.zeros(np.shape(left_image))
            ...
            #####
            lf_in_img = np.fft.fft2(left_image)
            #####
            ...
            ##### left image
            lf_in_img[0, 0:int(np.round(kx * (hl - 1)))] = lf_in_img[0, 0:int(np.round(kx * (hl - 1)))] / max_f
            lf_in_img[0:int(np.round(ky * (vl - 1))), 0] = lf_in_img[0:int(np.round(ky * (vl - 1))), 0] / max_f
			
            lf_in_img[0, int(np.round((0 - kx) * (hl - 1))): hl - 1] = lf_in_img[0,int(np.round((0 - kx) * (hl - 1))): hl - 1] / max_f # check(0,o)
            lf_in_img[0:int(np.round(ky * (vl - 1))), hl - 1] = lf_in_img[0:int(np.round(ky * (vl - 1))), hl - 1] / max_f
			
            lf_in_img[int(np.round((0 - ky) * (vl - 1))): v - 1, 0] = lf_in_img[int(np.round((0 - ky) * (vl - 1))): vl - 1, 0] / max_f
            lf_in_img[vl - 1, 0:int(np.round(kx * (hl - 1)))] = lf_in_img[v - 1, 0:int(np.round(kx * (hl - 1)))] / max_f
			
            lf_in_img[int(np.round((0 - ky) * (vl - 1))): vl - 1, hl - 1] = lf_in_img[int(np.round((0 - ky) * (vl - 1))): vl - 1,hl - 1] / max_f
            lf_in_img[vl - 1, int(np.round((0 - kx) * (hl - 1))): hl - 1] = lf_in_img[vl - 1,int(np.round((0 - kx) * (hl - 1))): hl - 1] / max_f
            #####


            out_img = np.zeros(np.shape(f_in_img), dtype=np.float32)
...
#            cv2.imshow("Peplography", out_img)
            cv2.imshow("IMAGE", right_image)

            out_img = out_img.dot(672) # 30000
            out_img = out_img.astype(np.uint8) # uint16

            out_img[:, :, 0] = cla.apply(out_img[:, :, 0])
            out_img[:, :, 1] = cla.apply(out_img[:, :, 1])
            out_img[:, :, 2] = cla.apply(out_img[:, :, 2])
...
#            cv2.imshow("Fourier Peplography", out_img)
            ...
            detections2 = detect(netMain, metaMain, image2, thresh)
            ...
            for detection in detections:
                ...
                distance_print = " distance - " + (str(distance) + "m")
                if format(label) == "person":
                   ...
                   log.info(pstring)
                   log.info(distance_print)

            log.info(chr(27) + "[2J"+"**** " + str(len(detections2)) + " Results ****")
            for detection in detections2:
                ...
                distance_print = " distance - " + (str(distance) + "m")
                if format(label) == "person":
                   cv2.rectangle(image2, (x_coord - thickness, y_coord - thickness),
                                 (x_coord + x_extent + thickness, y_coord + (18 + thickness*4)),
                                 color_array[detection[3]], -1)
                   cv2.putText(image2, label + " " +  (str(distance) + " m"),
                               (x_coord + (thickness * 4), y_coord + (10 + thickness * 4)),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                   cv2.rectangle(image2, (x_coord - thickness, y_coord - thickness),
                                 (x_coord + x_extent + thickness, y_coord + y_extent + thickness),
